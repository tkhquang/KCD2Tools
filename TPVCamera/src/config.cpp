...
void register_config_items()
{
    ...
    DMK::Config::register_atomic<bool>("Camera", "InteractFromCamera", "Interact From Camera", s.interact_from_camera, true);
    DMK::Config::register_atomic<float>("Camera", "ViewTransitionDuration", "View Transition Duration", s.view_transition_duration, 0.0f);
    ...
    DMK::Config::register_atomic<float>("Collision", "CollisionRadius", "Collision Radius", s.collision_radius, 0.15f);
    DMK::Config::register_atomic<float>("Collision", "ThinObstacleMaxSize", "Thin Obstacle Max Size", s.collision_thin_skip_max, 0.0f);
    ...
    DMK::Config::register_atomic<bool>("StateBehavior", "EnableStateBehavior", "Enable State Behavior", s.enable_state_behavior, true);
    DMK::Config::register_string("StateBehavior", "ForcedFPVState", "Forced FPV State",
                                 [&mask = s.forced_fpv_mask](const std::string &value)
    ...
    DMK::Config::register_string("StateBehavior", "ForcedTPVState", "Forced TPV State",
                                 [&mask = s.forced_tpv_mask](const std::string &value)
                                 { mask.store(parse_state_mask(value), std::memory_order_relaxed); },
                                 "");
    DMK::Config::register_string("StateBehavior", "OrbitExcludeState", "Orbit Exclude State",
                                 [&mask = s.orbit_exclude_mask](const std::string &value)
                                 { mask.store(parse_state_mask(value), std::memory_order_relaxed); },
                                 "Menu,Overlay,Cart,Combat,Mount,Minigame");
    DMK::Config::register_atomic<float>("StateBehavior", "StateSwitchHoldSeconds", "State Switch Hold Seconds", s.state_switch_hold_seconds, 0.2f);
    ...
    DMK::Config::register_string("StateBehavior", "SuppressTPVState", "Suppress TPV State",
    ...
    DMK::Config::register_atomic<float>("Presets", "PresetBlendSpeed", "Preset Blend Speed", s.preset_blend_speed, 8.0f);
    ...
    DMK::Config::register_key_combo("Camera", "ZoomInKey", "Zoom In Key",
    ...
    DMK::Config::register_key_combo("Camera", "ZoomOutKey", "Zoom Out Key",
                                    [](const DMK::Config::KeyComboList &c) {
