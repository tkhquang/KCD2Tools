...
struct RayHit
{
    float m_distance{0.0f};   ///< World distance from the ray origin to the hit.
    Vector3 m_point{};        ///< World-space hit position.
    Vector3 m_normal{};       ///< Surface normal at the hit.
    uintptr_t m_collider{0};  ///< IPhysicalEntity* of the entity hit (ray path only; 0 on the sphere path).
};
 ...
 */
[[nodiscard]] std::optional<RayHit> ray_world_intersection(const Vector3 &origin,
 ...
 */
[[nodiscard]] std::optional<RayHit> sphere_world_sweep(const Vector3 &origin, float radius,
                                                    const Vector3 &sweep, int objtypes,
                                                    const uintptr_t *p_skip_ents = nullptr,
                                                    int n_skip_ents = 0);
