...
static void capture_movement_input(const char **action_name, float value)
{
    if (action_name == nullptr ||
        !DMK::Memory::plausible_userspace_ptr(reinterpret_cast<uintptr_t>(action_name)))
    {
        return;
    }
                                                ...
                                                unsigned int activation, float value)
{
    ...
    return s_action_dispatch_original
               ? s_action_dispatch_original(self, action_name, activation, value)
               : 0;
}
