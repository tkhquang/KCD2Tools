...
namespace TPVCamera
{
    ...
    bool initialize_ui_menu_hooks()
    {
        ...
        try
        {
            ...
            auto open_result =
                hook_manager.create_inline_hook("MenuOpen", menu_open_addr, reinterpret_cast<void *>(menu_open_detour),
                                                reinterpret_cast<void **>(&s_menu_open_original), hook_config);

            if (!open_result.has_value())
            {
                throw std::runtime_error("Failed to create menu open hook: " +
                                         std::string(DMK::Hook::error_to_string(open_result.error())));
            }

            auto close_result = hook_manager.create_inline_hook(
                "MenuClose", menu_close_addr, reinterpret_cast<void *>(menu_close_detour),
                reinterpret_cast<void **>(&s_menu_close_original), hook_config);

            if (!close_result.has_value())
            {
                throw std::runtime_error("Failed to create menu close hook: " +
                                         std::string(DMK::Hook::error_to_string(close_result.error())));
            }
