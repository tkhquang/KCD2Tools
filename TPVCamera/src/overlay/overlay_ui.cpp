    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        if (s_rename_index == i && can_rename)
        {
            ...
            const bool committed = ImGui::InputText(
                "##rename", s_rename_buffer, sizeof(s_rename_buffer),
            ...
            if (committed)
            {
                if (!store.rename(i, std::string(s_rename_buffer)))
                {
                    DMK::Logger::get_instance().warning(
                        "Overlay: rename of preset index {} rejected (empty/duplicate/built-in).",
                        i);
                }
                cancel_rename();
            }
...
void draw_pin_toggle(PresetStore &store)
{
    ...
    hover_tooltip(
        "Preview the edited preset live regardless of the in-game state\n"
...
void draw_button_row(PresetStore &store, const CameraPreset &selected)
{
    ...
    ImGui::EndDisabled();
    hover_tooltip(is_builtin
                      ? "Built-in presets cannot be duplicated."
...
void draw_bind_editor(PresetStore &store, const CameraPreset &edited, int editing)
{
    ...
    {
        mask |= state_bit(GameState::Minigame) | state_bit(k_minigames[static_cast<std::size_t>(minigame_sel - 2)].bit);
    }
...
void draw_applied_follow_distance(int decimals)
{
    ...
    ImGui::EndDisabled();
    hover_tooltip(zoomed
                      ? "Clear the accumulated zoom so the camera returns to the configured Follow Distance above."
...
void draw_field_editors(PresetStore &store, CameraPreset &edited)
{
    ...
    for (const PresetField &field : Presets::fields())
    {
        const bool new_group =
            (current_group == nullptr) || (std::string_view(current_group) != field.group);
        if (new_group)
        {
