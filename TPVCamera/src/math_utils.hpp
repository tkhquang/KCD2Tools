...
struct Vector3
{
    ...
    Vector3 operator/(float scalar) const noexcept { return (scalar != 0.0f) ? Vector3(x / scalar, y / scalar, z / scalar) : Vector3(); }
...
struct Quaternion
{
    ...
    explicit Quaternion(float _x = 0.0f, float _y = 0.0f, float _z = 0.0f, float _w = 1.0f) noexcept : x(_x), y(_y), z(_z), w(_w) {}
