...
using RayWorldIntersectionFn = int(__fastcall *)(
    void *physical_world, const Vector3 *org, const Vector3 *dir, int objtypes,
    unsigned int flags, void *hits, int n_max_hits, void *skip_ents, int num_skip_ents,
    void *foreign_data, int foreign_index, const char *name_tag);
...
static int ray_world_intersection_guarded(void *physical_world, const Vector3 *org, const Vector3 *dir,
                                       int objtypes, unsigned int flags, void *hits,
                                       void *skip_ents, int n_skip_ents)
{
    __try
    {
        return s_ray_world_intersection(physical_world, org, dir, objtypes, flags, hits, 1,
                                      skip_ents, n_skip_ents, nullptr, 0, "TPVCameraRay");
    }
...
std::optional<RayHit> ray_world_intersection(const Vector3 &origin, const Vector3 &direction,
                                           int objtypes, unsigned int flags,
                                           const uintptr_t *skip_ents, int n_skip_ents)
{
...
using PrimitiveWorldIntersectionFn =
    float(__fastcall *)(void *physical_world, void *pp, void *p_lock_contacts, const char *name_tag);
...
std::optional<RayHit> sphere_world_sweep(const Vector3 &origin, float radius, const Vector3 &sweep,
                                      int objtypes, const uintptr_t *p_skip_ents, int n_skip_ents)
{
    ...
    const auto fn_slot =
        DMK::Memory::seh_read<uintptr_t>(*vtable + Constants::PHYS_WORLD_VTABLE_PWI_OFFSET);
    ...
    *reinterpret_cast<void **>(params + Constants::SPWI_OFF_LOCK_PRW) =
        params + Constants::SPWI_OFF_LOCK_IACTIVE;

    const float distance =
        primitive_world_intersection_guarded(fn, reinterpret_cast<void *>(world), params);
...
std::optional<RayHit> ray_fan_sweep(const Vector3 &origin, const Vector3 &sweep, float radius,
                                 int objtypes, unsigned int flags,
                                 const uintptr_t *skip_ents, int n_skip_ents)
{
    const float len = sweep.magnitude();
    ...
    const Vector3 offsets[5] = {
        Vector3{0.0f, 0.0f, 0.0f}, right * radius, right * (-radius), up * radius, up * (-radius)};
...
static float collider_min_extent(uintptr_t collider)
{
    ...
    if (!std::isfinite(sx) || !std::isfinite(sy) || !std::isfinite(sz) || sx < 0.0f || sy < 0.0f ||
        sz < 0.0f || sx > 1.0e5f || sy > 1.0e5f || sz > 1.0e5f)
    {
        return -1.0f;
    }
    float m = sx;
...
int resolve_player_physics_skip(const Vector3 &origin, const Vector3 &sweep, float radius,
                             ...
                             uintptr_t *out_skip, int max_out)
{
    ...
    for (const Vector3 &off : offsets)
    {
        ...
        const bool is_non_world =
            !h_world.has_value() || h_all->m_distance < h_world->m_distance - 0.02f;
        if (!is_non_world)
        {
            continue;
        }
